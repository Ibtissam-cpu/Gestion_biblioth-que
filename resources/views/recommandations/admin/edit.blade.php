...
<div class="bg-white shadow-md rounded-lg p-6">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-2xl font-bold">Modifier la recommandation</h1>
    </div>
    
    <form action="{{ route('recommandations.admin.update', $recommandation->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="livre_id" id="livre_id" required
                ...
                @foreach($livres as $livre)
                    <option value="{{ $livre->id }}" {{ old('livre_id', $recommandation->livre_id) == $livre->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <textarea name="commentaire" id="commentaire" rows="4" required
                class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('commentaire') border-red-500 @enderror">{{ old('commentaire', $recommandation->commentaire) }}</textarea>
            @error('commentaire')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="type" id="type" required
                ...
                <option value="general" {{ old('type', $recommandation->type) == 'general' ? 'selected' : '' }}>Générale (pour tous)</option>
                <option value="categorie" {{ old('type', $recommandation->type) == 'categorie' ? 'selected' : '' }}>Par catégorie</option>
                <option value="age" {{ old('type', $recommandation->type) == 'age' ? 'selected' : '' }}>Par tranche d'âge</option>
            </select>
            @error('type')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        ...
        <div id="categorieSection" class="mb-4 {{ $recommandation->type != 'categorie' ? 'hidden' : '' }}">
            ...
            <select name="categorie_id" id="categorie_id"
                ...
                @foreach($categories as $categorie)
                    <option value="{{ $categorie->id }}" {{ old('categorie_id', $recommandation->categorie_id) == $categorie->id ? 'selected' : '' }}>
                        {{ $categorie->nom }}
                    </option>
                @endforeach
            </select>
        ...
        <div id="ageSection" class="mb-4 {{ $recommandation->type != 'age' ? 'hidden' : '' }}">
            <label for="age_min" class="block text-gray-700 font-medium mb-2">Tranche d'âge</label>
            <div class="flex items-center gap-2">
                <input type="number" name="age_min" id="age_min" placeholder="Âge minimum" value="{{ old('age_min', $recommandation->age_min) }}"
                ...
                <span>à</span>
                <input type="number" name="age_max" id="age_max" placeholder="Âge maximum" value="{{ old('age_max', $recommandation->age_max) }}"
        ...
        <div class="mb-4">
            ...
            <select name="priorite" id="priorite" required
                ...
                <option value="1" {{ old('priorite', $recommandation->priorite) == '1' ? 'selected' : '' }}>Basse</option>
                <option value="2" {{ old('priorite', $recommandation->priorite) == '2' ? 'selected' : '' }}>Normale</option>
                <option value="3" {{ old('priorite', $recommandation->priorite) == '3' ? 'selected' : '' }}>Haute</option>
            </select>
            @error('priorite')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
